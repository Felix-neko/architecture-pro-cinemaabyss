...
export NEW_MOVIES_SERVICE_PERCENT=$((${MOVIES_MIGRATION_PERCENT:-0}))

# Проверяем процент
if [ "$NEW_MOVIES_SERVICE_PERCENT" -lt 0 ] || [ "$NEW_MOVIES_SERVICE_PERCENT" -gt 100 ]; then
    echo "[Kong Entrypoint] ПРЕДУПРЕЖДЕНИЕ: NEW_MOVIES_SERVICE_PERCENT должен быть от 0 до 100, установлен в 0"
    export NEW_MOVIES_SERVICE_PERCENT=0
fi
export OLD_MOVIES_SERVICE_PERCENT=$((100 - $NEW_MOVIES_SERVICE_PERCENT))

# Устанавливаем значения по умолчанию для URL сервисов
export MONOLITH_SERVICE_URL=${MONOLITH_SERVICE_URL:-localhost:8080}
export MOVIES_SERVICE_URL=${MOVIES_SERVICE_URL:-localhost:8081}
...
echo "- NEW_MOVIES_SERVICE_PERCENT: $NEW_MOVIES_SERVICE_PERCENT%"
echo "- OLD_MOVIES_SERVICE_PERCENT: $OLD_MOVIES_SERVICE_PERCENT%"

# И подставляем нужные переменные окружения в шаблон kong-конфигурации
echo "[Kong Entrypoint] Генерация конфигурации Kong..."
sed -e "s|\${OLD_MOVIES_SERVICE_PERCENT}|$OLD_MOVIES_SERVICE_PERCENT|g" \
    -e "s|\${NEW_MOVIES_SERVICE_PERCENT}|$NEW_MOVIES_SERVICE_PERCENT|g" \
    -e "s|\${MONOLITH_SERVICE_URL}|$MONOLITH_SERVICE_URL|g" \
    -e "s|\${MOVIES_SERVICE_URL}|$MOVIES_SERVICE_URL|g" \
